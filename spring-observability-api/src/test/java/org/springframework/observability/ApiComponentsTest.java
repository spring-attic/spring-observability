...
import org.springframework.observability.event.listener.composite.AllMatchingRecordingListener;
...
class ApiComponentsTest {
	...
	private final Recorder<CompositeContext> recorder = new SimpleRecorder<>(new AllMatchingRecordingListener(listener),
			clock);
	...
	void shouldRecordInstantEventWithProvidedTime() {
		Recorder<CompositeContext> recorder = new SimpleRecorder<>(new AllMatchingRecordingListener(listener), null);
		recorder.recordingFor(INSTANT_EVENT).highCardinalityName(INSTANT_EVENT.getLowCardinalityName() + "-12345")
