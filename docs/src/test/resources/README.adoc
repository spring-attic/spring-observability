...
Spring Observability Build is a common utility project for Spring Observability
...
Spring Observability Build publishes its `spring-observability-docs` module that contains
helpful scripts (e.g. README generation ruby script) and css, xslt and images
for the Spring Observability documentation. If you want to follow the same convention
...
<2> This plugin downloads the resources of the `spring-observability-docs` module
<3> This plugin unpacks the resources of the `spring-observability-docs` module
...
Spring Observability Build Docs comes with a set of attributes for asciidoctor that you can reuse.

[source,xml]
----
include::https://raw.githubusercontent.com/spring-projects/spring-observability/main/pom.xml[tags=attributes,indent=0]
----
